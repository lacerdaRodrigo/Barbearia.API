...
namespace Barbearia.API.Controllers
{
    ...
    public class AgendamentoController : ControllerBase
    {
        ...
        {
            var agendamentos = await _dbContext.Agendamentos.Include(a => a.Cliente).Include(b => b.Servicos).ToListAsync();
        ...
        public async Task<IActionResult> Edit(int id, Agendamento agendamentoDTO)
        {
            ...
            if (ModelState.IsValid)
            {
                var agendamento = _dbContext.Agendamentos.Include(x => x.Servicos).Include(b => b.Cliente).FirstOrDefault(x => x.AgendamentoID == agendamentoDTO.AgendamentoID);
                if (agendamento == null)
                {
                    return NotFound();
                }
        ...
        {
            var agendamento = await _dbContext.Agendamentos.Include(x => x.Servicos).Include(x => x.Cliente).FirstOrDefaultAsync(x => x.AgendamentoID == id);
            if (agendamento == null)
            {
                return NotFound();
            }
